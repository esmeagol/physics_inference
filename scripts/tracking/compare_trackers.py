...
# Add the project root directory to the path to allow imports
...
try:
    ...
    from src.tracking.trackers.deepsort_tracker import DeepSORTTracker
    from src.tracking.trackers.sv_bytetrack_tracker import SVByteTrackTracker
...
def load_ground_truth_events(file_path: str) -> Optional[List[Dict[str, Any]]]:
    """
    Load ground truth events from JSON file.
    
    Args:
        file_path: Path to the JSON file containing ground truth events
        
    Returns:
        List of ground truth events, or None if there was an error
    """
    try:
        with open(file_path, 'r') as f:
            events = json.load(f)
            if not isinstance(events, list):
                print(f"Error: Expected list of events, got {type(events).__name__}")
                return None
                
            # Ensure all events are dictionaries
            if not all(isinstance(event, dict) for event in events):
                print("Error: Not all events are dictionaries")
                return None
...
def create_sample_ground_truth_events() -> List[Dict[str, Any]]:
    """
...
Example usage:
  ...
  python compare_trackers.py --video snooker.mp4 --visualize --debug-tracking
    ...
    if use_ground_truth and ground_truth_events is not None:
        try:
    ...
    for tracker_name in args.trackers:
        ...
        if tracker_name in tracker_map:
            ...
            print(f"Warning: MOLT tracker not available. Install tracking.trackers.molt package. Skipping.")
        else:
    ...
    if use_ground_truth and hasattr(benchmark, 'ground_truth_results') and benchmark.ground_truth_results:
        ...
        for tracker_name, gt_result in benchmark.ground_truth_results.items():
            if gt_result is not None and hasattr(benchmark, 'ground_truth_evaluator') and benchmark.ground_truth_evaluator is not None:
                try:
