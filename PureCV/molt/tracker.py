...
class MOLTTracker(Tracker[Dict[str, Any]]):
    ...
    def init(self, frame: Frame, detections: List[Detection]) -> bool:
        ...
        Args:
            frame (numpy.ndarray): First frame as numpy array with shape (H, W, C) 
    ...
    def update(self, frame: Frame, detections: Optional[List[Detection]] = None) -> List[Track]:
        ...
        Args:
            frame (numpy.ndarray): New frame as numpy array with shape (H, W, C) 
                in BGR format. Must be a valid 3-channel color image.
    ...
    def visualize(self, frame: Frame, tracks: Sequence[Track], 
        ...
        Args:
            frame (numpy.ndarray): Input frame as numpy array with shape (H, W, C) 
        ...
        Returns:
            numpy.ndarray: A copy of the input frame with comprehensive tracking
    ...
    def get_population_statistics(self) -> Dict[str, Any]:
        """
        Get comprehensive statistics about all tracker populations.
        ...
        Example:
            ...
            >>> print(f"Average best weight: {stats['average_best_weight']:.3f}")
        """
        if not self.populations:
            return {
                'total_populations': 0,
                'active_populations': 0,
                'average_best_weight': 0.0,
                'average_population_size': 0.0,
                'population_details': []
            }
        
        active_populations = [p for p in self.populations if p.best_tracker is not None]
        total_best_weight = sum(p.best_tracker.total_weight for p in active_populations)
        ...
        return {
            ...
            'average_best_weight': total_best_weight / len(active_populations) if active_populations else 0.0,
            'average_population_size': total_population_size / len(self.populations) if self.populations else 0.0,
            'population_details': [p.get_population_statistics() for p in self.populations]
        }
            ...
            if not counts_valid:
                ...
                for ball_class, violation_info in violations.items():
                    ...
                    if violation_type == 'over_count':
                        ...
                        track_ids = [t['id'] for t in class_tracks]
    ...
    def get_ball_count_statistics(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Ball count statistics and violation information
        """
        return self.ball_count_manager.get_statistics()
    ...
    def get_performance_metrics(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict[str, Any]: Comprehensive performance metrics including
                          population statistics, tracking accuracy, and efficiency
        """
        metrics = {
            'frame_count': self.frame_count,
            'total_populations': len(self.populations),
            'active_populations': len([p for p in self.populations if p.best_tracker is not None]),
            'total_tracks_created': self.total_tracks_created,
            'total_tracks_lost': self.total_tracks_lost,
            'total_fragmentations': self.total_fragmentations,
        ...
        if self.populations:
            ...
            for population in self.populations:
                ...
                total_trackers += pop_stats['population_size']
                total_best_weight += pop_stats['best_weight']
                total_avg_weight += pop_stats['average_weight']
            
            metrics.update({
                'population_metrics': population_metrics,
                'total_trackers': total_trackers,
                'avg_best_weight': total_best_weight / len(self.populations),
                'avg_population_weight': total_avg_weight / len(self.populations),
                'avg_trackers_per_population': total_trackers / len(self.populations)
            })
        
        # Ball count metrics
        if hasattr(self, 'ball_count_manager'):
            ball_metrics = self.ball_count_manager.get_statistics()
            metrics['ball_count_metrics'] = ball_metrics
    ...
    def get_tracking_status(self) -> Dict[str, Any]:
        ...
        elif len(active_populations) < len(self.populations) * 0.5:
        ...
        if active_populations:
            avg_confidence = sum(p.best_tracker.total_weight for p in active_populations) / len(active_populations)
        ...
        status['active_populations'] = len(active_populations)
        status['total_populations'] = len(self.populations)
