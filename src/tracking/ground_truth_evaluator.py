...
from typing import Dict, List, Optional, Any, Tuple, cast, Union, TYPE_CHECKING
from dataclasses import dataclass
import math
import sys
from pathlib import Path

# Import type stubs first to avoid redefinition errors
if TYPE_CHECKING:
    from .event_processor import EventProcessor as _EventProcessor
    from .event_processor import ProcessedEvent as _ProcessedEvent
    from .state_reconstructor import StateReconstructor as _StateReconstructor
    from .moment_evaluator import (
        MomentEvaluator as _MomentEvaluator,
        MomentEvaluation as _MomentEvaluation,
        ErrorType as _ErrorType
    )
else:
    # At runtime, try to import the actual implementations
    try:
        from .event_processor import EventProcessor as _EventProcessor
        from .event_processor import ProcessedEvent as _ProcessedEvent
        from .state_reconstructor import StateReconstructor as _StateReconstructor
        ...
        try:
            ...
            # Import type stubs as a last resort
            from typings.event_processor import EventProcessor as _EventProcessor
            from typings.event_processor import ProcessedEvent as _ProcessedEvent
            from typings.state_reconstructor import StateReconstructor as _StateReconstructor
            from typings.moment_evaluator import (
                MomentEvaluator as _MomentEvaluator,
                MomentEvaluation as _MomentEvaluation,
                ErrorType as _ErrorType
            )
...
ProcessedEvent = _ProcessedEvent
StateReconstructor = _StateReconstructor
MomentEvaluator = _MomentEvaluator
MomentEvaluation = _MomentEvaluation
...
class GroundTruthEvaluator:
    ...
    def _analyze_temporal_patterns(self) -> Dict[str, Any]:
        ...
        accuracy_over_time: List[Optional[float]] = []
        error_counts_over_time: List[int] = []
    ...
    def _analyze_accuracy_trend(self, accuracy_values: List[Optional[float]]) -> Dict[str, Any]:
        ...
        """
        # Filter out None values - type checker knows these are float after filter
        ...
        # Use explicit float conversion to ensure type safety
        numerator = sum(float((x - x_mean) * (y - y_mean)) for x, y in zip(x_values, valid_accuracies))
        denominator = sum(float((x - x_mean) ** 2) for x in x_values)
        
        if abs(denominator) < 1e-10:  # Avoid division by zero with a small epsilon
            slope = 0.0
        else:
    ...
    def validate_tracker_output_format(self, tracker_results: List[Dict[str, Any]]) -> List[str]:
        ...
        if not isinstance(tracker_results, list):
            return ["Tracker results must be a list"]  # type: ignore[unreachable]
            
        # Validate each result
        for i, result in enumerate(tracker_results):
            if not isinstance(result, dict):
                errors.append(f"Result {i}: Must be a dictionary")  # type: ignore[unreachable]
                continue
            
            # Check required fields
            self._validate_required_fields(result, i, errors)
            
            # Check optional fields  
            self._validate_optional_fields(result, i, errors)
        
        return errors
    
    def _validate_required_fields(self, result: Dict[str, Any], index: int, errors: List[str]) -> None:
        """Validate required fields in a result."""
        if 'timestamp' not in result:
            errors.append(f"Result {index}: Missing 'timestamp' field")
        elif not isinstance(result['timestamp'], (int, float)):
            errors.append(f"Result {index}: 'timestamp' must be a number")
    
    def _validate_optional_fields(self, result: Dict[str, Any], index: int, errors: List[str]) -> None:
        """Validate optional fields in a result."""
        if 'counts' in result and not isinstance(result['counts'], dict):
            errors.append(f"Result {index}: 'counts' must be a dictionary")
        
        if 'detections' in result and not isinstance(result['detections'], list):
            errors.append(f"Result {index}: 'detections' must be a list")
    ...
    def _analyze_error_distribution(self) -> Dict[str, Any]:
        ...
        """
        # Initialize error_distribution with minimal type hints
        from typing import Dict, Any
        
        # Initialize the dictionary with explicit type comments
        error_distribution = {
            'count_errors': {
                'over_detection': 0,
                'under_detection': 0,
    ...
    def _analyze_transition_periods(self) -> Dict[str, Any]:
        ...
        """
        # Type hint for transition_analysis to help mypy
