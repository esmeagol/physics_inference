...
class GroundTruthVisualizer:
    ...
    def create_comparative_analysis(self, tracker_summaries: Dict[str, EvaluationSummary],
        ...
        all_ball_types: set[str] = set()
        for summary in tracker_summaries.values():
            all_ball_types.update(summary.per_ball_accuracy.keys())
        all_ball_types_sorted = sorted(list(all_ball_types))
        ...
        for tracker_name in tracker_names:
            ...
            row = []
            for ball_type in all_ball_types_sorted:
        ...
        ax3.set_xticks(range(len(all_ball_types_sorted)))
        ax3.set_xticklabels(all_ball_types_sorted, rotation=45)
        ax3.set_yticks(range(len(tracker_names)))
        ax3.set_yticklabels(tracker_names)
        
        # Add text annotations
        for i in range(len(tracker_names)):
            for j in range(len(all_ball_types_sorted)):
    ...
    def _create_error_radar_chart(self, ax: Any, tracker_summaries: Dict[str, EvaluationSummary]) -> None:
    ...
    def _plot_event_timeline(self, ax: Any, events: List[Any]) -> None:
    ...
    def create_summary_dashboard(self, tracker_summaries: Dict[str, EvaluationSummary],
        ...
        all_ball_types_2: set[str] = set()
        for summary in tracker_summaries.values():
            all_ball_types_2.update(summary.per_ball_accuracy.keys())
        all_ball_types_sorted_2 = sorted(list(all_ball_types_2))
        
        accuracy_matrix = []
        for tracker_name in tracker_names:
            summary = tracker_summaries[tracker_name]
            row = []
            for ball_type in all_ball_types_sorted_2:
                if ball_type in summary.per_ball_accuracy:
                    row.append(summary.per_ball_accuracy[ball_type]['accuracy'])
                else:
                    row.append(0)
        ...
        ax3.set_xticks(range(len(all_ball_types_sorted_2)))
        ax3.set_xticklabels(all_ball_types_sorted_2, rotation=45)
        ax3.set_yticks(range(len(tracker_names)))
        ax3.set_yticklabels(tracker_names)
        
        # Add text annotations
        for i in range(len(tracker_names)):
            for j in range(len(all_ball_types_sorted_2)):
        ...
        colors = ['orange', 'red', 'purple', 'brown']
        pie_result = ax4.pie(total_errors, labels=error_categories, colors=colors, 
                            autopct='%1.1f%%', startangle=90)
