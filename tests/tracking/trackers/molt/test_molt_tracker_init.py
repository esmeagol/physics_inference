...
def create_test_frame(width: int = 640, height: int = 480) -> NDArrayUInt8:
    """
    ...
    Returns:
        NDArrayUInt8: Test frame in BGR format
    """
    frame: NDArrayUInt8 = np.zeros((height, width, 3), dtype=np.uint8)
...
def create_test_detections() -> List[Dict[str, Any]]:
    ...
    return [
        {
            'class': 'red',
            'bbox': [100, 100, 30, 30],  # x, y, width, height
            'confidence': 0.95
        },
        {
            'class': 'white',
